...
Component({
    ...
    didUpdate: function didUpdate(prevProps, prevData) {
        ...
        if (prevData.width > 0 && prevData.height > 0) {
            console.log(`prevData width=${prevData.width}, height=${prevData.height}`);
            return;
        }

        if (this.data.width <= 0 || this.data.height <=0 ) {
            console.log(`this.data width=${this.data.width}, height=${this.data.height}`);
            return;
        }

        //just draw at first time
        var me = this;
        var canvasId = me.props.canvasId;
        var pixelRatio = dd.getSystemInfoSync().pixelRatio;
        if (pixelRatio <= 0 ){
            console.log(`invalid pixelRatio ${pixelRatio}, reset to 1`);
            pixelRatio = 1;
        }
        var canvasWidth = this.data.width / pixelRatio;
        var canvasHeight = this.data.height / pixelRatio;
